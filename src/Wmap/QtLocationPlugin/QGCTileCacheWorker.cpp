...
//QGCReadTileWorker::QGCReadTileWorker(QSqlDatabase *db, QGCMapTask *task) : _db(db), _task(task)
//{
//	setAutoDelete(true);
//}
//
//QGCReadTileWorker::~QGCReadTileWorker()
//{
//	_task->deleteLater();
//}
//
//void QGCReadTileWorker::run()
//{
//	if (NULL == _task)
//		return;
//	bool found = false;
//
//	QGCFetchTileTask* task = static_cast<QGCFetchTileTask*>(_task);
//	QSqlQuery query(*_db);
...
QGCCacheWorker::QGCCacheWorker()
	...
	, _memoryUsed(0)
{
	//_tileReader.setMaxThreadCount(8);
}
...
//-----------------------------------------------------------------------------
void QGCCacheWorker::setMaxMemoryUse(quint32 memory)
{
	_memory = memory * 1024;
}
...
QGCCacheWorker::_saveTile(QGCMapTask *mtask)
{
	...
	if (_valid) {
		...
		int leng = tile->img().size() + sizeof(QGCCacheTile);
		if (_memory < leng)
			return;

		while (_memoryUsed + leng > _memory)
		{
			if (_tileMap.size() == 0)
				return;

			// 移除第一个进入的瓦片
			QString hash = _tileList.front();
			_tileList.pop_front();

			QGCCacheTile *tileTmp = _tileMap[hash];
			_tileMap.remove(hash);

			if (tileTmp)
			{
				_memoryUsed = _memoryUsed - sizeof(QGCCacheTile) - tileTmp->img().size();
				if (_memoryUsed < 0)
					_memoryUsed = 0;
		...
		// 添加新的瓦片
		_tileList.push_back(tile->hash());
		_tileMap[tile->hash()] = tile;
		_memoryUsed += leng;
	}
}
...
{     
	//QGCReadTileWorker *worker = new QGCReadTileWorker(_db, mtask);
	//_tileReader.start(worker);
	if (!_testTask(mtask))
		return;
	bool found = false;
...
QGCCacheWorker::_getTile(QGCMapTask* mtask)
{
	...
	bool found = false;
	QGCFetchTileTask* task = static_cast<QGCFetchTileTask*>(mtask);
	QString hash = task->hash();
	if (_tileMap.contains(hash))
	{
		QGCCacheTile* tile = new QGCCacheTile(task->hash(), _tileMap[hash]->img(), _tileMap[hash]->format(), _tileMap[hash]->type());
		task->setTileFetched(tile);
	}
	if (!found) {
		//qCDebug(QGCTileCacheLog) << "_getTile() (NOT in DB) HASH:" << task->hash();
...
{
    //qDebug()<<"____________getTileSets";
    if(!_testTask(mtask)) {
        return;
    }
...
{
    if(_valid) {
       // qDebug()<<"_____________createTileSet1:";
        //-- Create Tile Set
        ...
        if(!query.exec()) {
           ...
           // qDebug()<<"_____________createTileSet2:";
            ...
            for(int z = task->tileSet()->minZoom(); z <= task->tileSet()->maxZoom(); z++) {
                ...
                for(int x = set.tileX0; x <= set.tileX1; x++) {
                    for(int y = set.tileY0; y <= set.tileY1; y++) {
                        ...
                        //qDebug()<<"_____________createTileSet2-hash:"<<hash;
            ...
            _db->commit();
            //-- Done
            //qDebug()<<"_____________createTileSet2-Done:";
...
{
    //qDebug()<<"___________QGCCacheWorker_init";
    _failed = false;
...
#else
    if(!_hostLookupID) {
        _hostLookupID = QHostInfo::lookupHost("www.baidu.com", this, SLOT(_lookupReady(QHostInfo)));
        //qDebug()<<"____________________baidu";
    }
#endif
*/
}
...
{
    //qDebug()<<"____________________lookupReady";
#if defined(__android__)
    Q_UNUSED(info);
#else
    _hostLookupID = 0;
