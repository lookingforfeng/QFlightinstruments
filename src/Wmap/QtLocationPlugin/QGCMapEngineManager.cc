...
void
QGCMapEngineManager::init()
{
    QQmlEngine::setObjectOwnership(this, QQmlEngine::CppOwnership);
    qmlRegisterUncreatableType<QGCMapEngineManager>("MyMapEngineManager", 1, 0, "MyMapEngineManager", "Reference only");
...
QGCMapEngineManager::updateForCurrentView(double lon0, double lat0, double lon1, double lat1, int minZoom, int maxZoom, const QString& mapName)
{
    ...
    //qDebug()<<"____________updateForCurrentView:"<<lon0<<lat0;
}
...
{
   // qDebug()<<"____________loadTileSets";
    if(_tileSets.count()) {
        _tileSets.clear();
...
QGCMapEngineManager::startDownload(const QString& name, const QString& mapType)
{
    ...
    if (mapType != "Airmap Elevation Data" && _fetchElevation) {
        ...
        //qWarning() <<  "QGCMapEngineManager::startDownload() _fetchElevation No Tiles to save";
    }
}
