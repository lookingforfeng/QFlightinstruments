...
static const char* kDbFileName = "WMapCache.db";
...
stQGeoTileCacheQGCMapTypes kMapTypes[] = {
    ...
    {"Gaode Street Map",     UrlFactory::GaodeStreet},
...
#else
	QString cacheDir;
    cacheDir = QStandardPaths::writableLocation(QStandardPaths::GenericCacheLocation) + QStringLiteral("/WMapCache" CACHE_PATH_VERSION);
	 
	//cacheDir = QStringLiteral("D:/") + QStringLiteral("/WMapCache" CACHE_PATH_VERSION);
	//QMessageBox msgbox;
	//msgbox.setText(cacheDir);
	//msgbox.exec();
	
#endif
    if(!QDir::root().mkpath(cacheDir)) {
        qWarning() << "Could not create mapping disk cache directory: " << cacheDir;
       // qDebug()<<"manager3:"<<cacheDir;
        cacheDir = QDir::homePath() + QStringLiteral("/.qgcmapscache/");
        if(!QDir::root().mkpath(cacheDir)) {
            qWarning() << "Could not create mapping disk cache directory: " << cacheDir;
           // qDebug()<<"manager4:"<<cacheDir;
            cacheDir.clear();
        }
    }
    _cachePath = cacheDir;
