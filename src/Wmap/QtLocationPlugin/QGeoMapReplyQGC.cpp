...
//-----------------------------------------------------------------------------
QGeoTiledMapReplyQGC::QGeoTiledMapReplyQGC(QNetworkAccessManager *networkManager,
	const QNetworkRequest &request, const QGeoTileSpec &spec, QObject *parent)
...
//-----------------------------------------------------------------------------
void
QGeoTiledMapReplyQGC::localReplyFinished()
{
	_timer.stop();
	if (!_local) {
		emit aborted();
...
//-----------------------------------------------------------------------------
void
QGeoTiledMapReplyQGC::localReplyError()
{
	_timer.stop();
	if (!_local) {
		return;
	...
	//-- Regular map tile
	qWarning() << "Fetch tile error:" << _local->errorString();

	QFile b(":/res/notile.png");
	if (b.open(QFile::ReadOnly))
	{
		QByteArray a = b.readAll();
		setMapImageData(a);
	}
	setFinished(true);
...
QGeoTiledMapReplyQGC::cacheError(QGCMapTask::TaskType type, QString /*errorString*/)
{
    if(!getQGCMapEngine()->isInternetActive()) {
       // qDebug()<<"_______getQGCMapEngineisInternetActive"<<(getQGCMapEngine()->isInternetActive());
        if ((UrlFactory::MapType)tileSpec().mapId() == UrlFactory::MapType::AirmapElevation) {
...
//-----------------修改为默认从本地获取，屏蔽了网络获取接口 modify by chjm------------------------
void
QGeoTiledMapReplyQGC::cacheError(QGCMapTask::TaskType type, QString /*errorString*/)
{
	if (type != QGCMapTask::taskFetchTile) {
	...
	_local = new QGeoTileLocalReply((UrlFactory::MapType)_tileSpec.mapId(), _tileSpec.x(), _tileSpec.y(), _tileSpec.zoom());

	connect(_local, &QGeoTileLocalReply::finished, this, &QGeoTiledMapReplyQGC::localReplyFinished);
	connect(_local, &QGeoTileLocalReply::error, this, &QGeoTiledMapReplyQGC::localReplyError);

	QGeoTileLocalManager::instance()->start(_local);
